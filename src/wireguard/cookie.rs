...
pub fn cookie_reply(psk: Option<&[u8; 32]>,
    ...
    // Receiver index.
    out.extend_from_slice(peer_index.as_slice());
    ...
    // Encrypted cookie.
    let encrypted_cookie = encrypt(temp.as_bytes(), &nonce, mac1, cookie);
    out.extend_from_slice(&encrypted_cookie);
...
pub fn process_cookie_reply(psk: Option<&[u8; 32]>,
    ...
    cookie.copy_from_slice(&decrypt(temp.as_bytes(), nonce, mac1, ciphertext)?);
    Ok(cookie)
}

pub fn cookie_sign(m: &mut Vec<u8>, cookie: Option<&Cookie>) {
    if cookie.is_some() {
        let mac2 = blake2s(16, cookie.unwrap(), m);
        m.extend_from_slice(mac2.as_bytes());
    } else {
