...
///
/// Does not include `mac2`, but has capacity reserved for it.
///
/// Returns: Message, noise handshake state.
pub fn initiate(wg: &WgInfo, peer: &PeerInfo, self_index: Id) -> (Vec<u8>, HS) {
    ...
    msg.extend_from_slice(&[1, 0, 0, 0]);
    // Self index.
    msg.extend_from_slice(self_index.as_slice());

    // Noise part: e, s, timestamp.
    let timestamp = TAI64N::now();
    msg.extend_from_slice(&hs.write_message(&timestamp.to_external()));

    // Mac1.
    let mac1 = mac(wg.psk.as_ref(),
                   &[peer.peer_pubkey.as_slice(), msg.as_slice()]);
...
///
/// Expect message without `mac2`, of length 132.
pub fn process_initiation(wg: &WgInfo, msg: &[u8]) -> Result<InitProcessResult, ()> {
    ...
    let timestamp = hs.read_message(&msg[8..116]).map_err(|_| ())?;
...
///
/// Does not include `mac2`, but has capacity reserved for it.
pub fn responde(wg: &WgInfo, result: &mut InitProcessResult, self_id: Id) -> Vec<u8> {
    let mut response = Vec::with_capacity(92);
    // Type and zeros.
    response.extend_from_slice(&[2, 0, 0, 0]);

    response.extend_from_slice(self_id.as_slice());
    response.extend_from_slice(result.peer_id.as_slice());

    let mut hs = &mut result.handshake_state;

    let noise_response = hs.write_message(&[]);
    response.extend_from_slice(&noise_response);

    let mac1 = mac(wg.psk.as_ref(), &[hs.get_rs().as_ref().unwrap(), &response]);
    response.extend_from_slice(&mac1);
...
/// Returns peer index.
///
/// Expect message without `mac2`, of length 76.
pub fn process_response(wg: &WgInfo, hs: &mut HS, msg: &[u8]) -> Result<Id, ()> {
    ...
    hs.read_message(&msg[12..60]).map_err(|_| ())?;
